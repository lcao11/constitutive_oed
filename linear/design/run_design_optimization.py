"""Bayesian optimization driver for linear OED design with MPI.

Usage:
    mpirun -n <ranks> python linear/design/run_design_optimization.py \
        --output_path ./design_result/

Expected output:
    - CSV log and JSON checkpoints in the output directory.
"""
...
class UtilityFunction:
    ...
    def generate_parameter_samples(self, seed):
        ...
        if rank == 0:
            ...
            # Transform to standard normal distribution N(0, I) via inverse CDF (probit).
...
def main():
    ...
    for i in range(model_settings["n_control_points"]):
        physical_bounds[f"x{3+i}"] = (0.0, control_upper)
    ...
    def eval_point(pdict_norm):
        ...
        rotation = pdict["x0"]
        stretch = np.array([pdict["x1"], pdict["x2"]], dtype=float)
        controls = [pdict[f"x{3+i}"] for i in range(model_settings["n_control_points"])]
    ...
    def save_state(tag, counter):
        ...
        state = {
            ...
            "acquisition_iteration": acq_iteration,
        }
