...
def load_dataset_group(path: str, seed: int, num_files: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Loads a group of pickle files defined by a seed and a range of process IDs.
...
def filter_non_positive_fims(fims: np.ndarray, params: np.ndarray, designs: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
