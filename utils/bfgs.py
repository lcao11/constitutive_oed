...
from hippylib.algorithms.NewtonCG import LS_ParameterList
...
class BFGS_operator:
    ...
    def solve(self, x, b):
        ...
        """
        A = []
        if self.help_2 is None:
            self.help_2 = b.copy()
        else:
            self.help_2.zero()
            self.help_2.axpy(1.0, b)

        for s, y, r in zip(reversed(self.S), reversed(self.Y), reversed(self.R)):
            a = r * s.inner(self.help_2)
            A.append(a)
            self.help_2.axpy(-a, y)

        self.H0inv.solve(x, self.help_2)     # x = H0 * x_copy
    ...
    def update(self, s, y):
        ...
        if self.help_1 is None:
            self.help_1 = y.copy()
        else:
            self.help_1.zero()

        sy = s.inner(y)
        self.solve(self.help_1, y)
        yHy = y.inner(self.help_1)
        theta = 1.0
        if sy < damp*yHy:
            theta = (1.0-damp)*yHy/(yHy-sy)
            s *= theta
            s.axpy(1-theta, self.help_1)
            sy = s.inner(y)
        if sy <= 0.:
        ...
        if len(self.S) > memlim:
            ...
            self.Y.pop(0)
            self.R.pop(0)
            # self.update_scaling = True # Removed: we update every time now

        # re-scale H0 based on most recent secant information
        if hasattr(self.H0inv, "d0"):
            s_last  = self.S[-1]
            y_last = self.Y[-1]
            d0 = s_last.inner(y_last) / y_last.inner(y_last)
            self.H0inv.d0 = d0
            # self.update_scaling = False # Removed
...
class BFGS:
    ...
    def solve(self, x, H0inv, bounds_xPARAM=None, out_frequency=None):
        """
        ...
        if out_frequency is not None:
            x_history = []
            x_history.append([x[i].copy() for i in range(len(x))])
        ...
        while (self.it < max_iter) and (self.converged == False):
            ...
            if out_frequency is not None:
                if self.it==0 or (self.it+1)%out_frequency == 0:
        ...
        if out_frequency is None:
            ...
            return x, x_history, cost_history, gradnorm_history